...
var domElem = this.getElement().$;

// Render content selector.
currentContentSelector = new ContentSelector();
currentContentSelector.render(domElem);

// Render radio group.
currentRadioGroup = new RadioGroup.RadioGroup();
currentRadioGroup.setValue(0);
domElem.appendChild(currentRadioGroup.element);
currentRadioGroup.
  setOptions([{title: "Inline", value: 0}, {title: "Pop-out", value: 1}, {title: "Modal", value: 2}]);
...
if (currentContentSelector != null) {
  currentContentSelector.dispose();
  currentContentSelector = null;
  currentRadioGroup.element.parentElement.removeChild(currentRadioGroup.element);
...
if (data.type == "anchor") {
  data.type = "url";
  data.url = {url: currentContentSelector.getSelected().getGoogUri().toString() };

  if (currentRadioGroup.getValue() == 1 ) {
	data.target = {type: "_blank", name: "_blank"};
  }
...
console.log("elements set", attributes.set );
