...
( function()
{
	...
	function addCombo( editor, comboName, styleType, lang, entries, defaultLabel, styleDefinition, order )
  {
		...
		for ( var i = 0; i < names.length; i++ )
    {
        ...
        applyFontStyleValue(editor, styles, value, this.getValue() );
			},
		...
		init: function( editor )
    {
      ...
      AI_FONTSIZE_STYLES = addCombo( editor, "FontSize", "size", editor.lang.font.fontSize, config.fontSize_sizes, config.fontSize_defaultLabel, config.fontSize_style, 40 );

      // AI LABS PATCH -- Add FontSizeUp/Down
      editor.addCommand("fontSizeUp", {
      exec: function()
      {
        fontSizeIncrease(editor, AI_FONTSIZE_STYLES, 1);
      }
      });

      editor.ui.addButton("FontSizeUp", {
      label: "Increase font size",
      command: "fontSizeUp"});

      editor.addCommand("fontSizeDown", {
      exec: function() {
        fontSizeIncrease(editor, AI_FONTSIZE_STYLES, -1);
      }
      });

      editor.ui.addButton("FontSizeDown", {
      label: "Decrease font size",
      command: "fontSizeDown"});
     // AI LABS PATCH END -- Add FontSizeUp/Down
		}
	} );
} )();
...
function applyFontStyleValue(editor, styles, value, previousValue)
{
  editor.focus();
...
// AI LABS PATCH BEGIN: Relative size change
